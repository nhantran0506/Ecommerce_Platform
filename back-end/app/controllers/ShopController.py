...
class ShopController:
    ...
    async def statistics_revenue(self, shop_data: ShopGetData, current_user: User):
        try:
            ...
            cache_key = f"shop:{shop.shop_id}:revenue_stats:{timestamp_dt.strftime('%Y-%m')}"
            ...
            # Get shop's products
            shop_products_query = select(ShopProduct).where(ShopProduct.shop_id == shop.shop_id)
    ...
    async def statistics_top_products(self, shop_data: ShopGetData, current_user: User):
        try:
            ...
            timestamp_dt = shop_data.timestamp
            cache_key = f"shop:{shop.shop_id}:top_products:{timestamp_dt.strftime('%Y-%m')}"
    ...
    async def statistics_categories(self, shop_data: ShopGetData, current_user: User):
        try:
            ...
            timestamp_dt = shop_data.timestamp
            cache_key = f"shop:{shop.shop_id}:category_stats:{timestamp_dt.strftime('%Y-%m')}"
            ...
            for product_id in product_ids:
                ...
                cat_query = select(CategoryProduct).where(CategoryProduct.product_id == product_id)
                cats = await self.db.execute(cat_query)
                cats = cats.scalars().all()

                for cat in cats:
                    cat_name_query = select(Category).where(Category.cat_id == cat.cat_id)
