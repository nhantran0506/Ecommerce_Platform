...
class User(Base):
    ...
    role: Mapped[String] = mapped_column(
        Enum(UserRoles), nullable=False, default=UserRoles.USER
    )
    ...
    authenticate: Mapped["Authentication"] = relationship(
        "Authentication", back_populates="user"
    )
    product_ratings: Mapped[list["ProductRating"]] = relationship(
        "ProductRating", back_populates="user"
    )
    shop_ratings: Mapped[list["ShopRating"]] = relationship(
        "ShopRating", back_populates="user"
    )
